        ...
        try {
            ...
            if ($primaryAccount) {
                ...
                if ($betaUser) {
                    ...
                    elseif ($adminEmployeeId -and [string]::IsNullOrEmpty($betaUser.employeeId)) {
                        # If primary account has empty employeeId but matches by UPN pattern, consider it a match
                        Write-Host "  Found matching primary account by UPN pattern with empty employeeId: $($primaryAccount.UserPrincipalName)" -ForegroundColor Green
                        $primaryAccount | Add-Member -NotePropertyName "EmployeeId" -NotePropertyValue $adminEmployeeId -Force
                        return $primaryAccount
                    }
                    else {
